...
這是一個使用 discord 機器人達成的交談式表單系統，可以在 discord 上完成類似於Google表單的操作，更可當成審核系統使用。

使用者填寫的表單資料會儲存至 google sheets 方便管理者整理與歸納，更可以是當成一個審核系統使用

使用者可以對已發出去的表單進行刪除、查詢
...
```
`t` 表單命令，填寫表單需要使用命令前墜加上命令以填寫，預設前墜為 `-` 所以要填寫該表單就要使用 `-t` 命令。   
